...
def compute_trustworthiness(dataset, visu, projection_K, dataset_K, I_dataset):
	...
	for i in range(0, N):
		...
		for j in UK_i:
			# print('j', j)
			# print(I_dataset.shape)
			# print(I_dataset[i, :])
			# print(np.where(I_dataset[i, :] == j))
			# print(len(np.where(I_dataset[i, :] == j)))
			if len(np.where(I_dataset[i, :] == j)[0])>0:
...
def compute_continuity(dataset, visu, projection_K, dataset_K, I_projection):
	...
	for i in range(0, N-1):
		...
		for j in VK_i:
			if len(np.where(I_projection[i, :] == j)[0])>0:
